...
//    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewEdit::OnStartFrame, this, XRCID("StartFrame"));

    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewEdit::OnMachineOne, this, XRCID("Machine1"));
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewEdit::OnMachineTwo, this, XRCID("Machine2"));
...
///**
// * Change frame for machine adapter
// * @param event The menu event
// */
//void ViewEdit::OnStartFrame(wxCommandEvent& event)
//{
//    std::shared_ptr<Drawable> machineAdaptorOne;
//    std::shared_ptr<Drawable> machineAdaptorTwo;
//    for(auto actor :GetPicture()->GetActors())
//    {
//        if(actor->GetName() == "MachineOne")
//        {
//            for(auto drawable : actor->GetDrawables())
//            {
//                if(drawable->GetName() == L"MachineAdapterOne")
//                {
//                    machineAdaptorOne = drawable;
//                }
//                if(drawable->GetName() == L"MachineAdapterTwo")
//                {
//                    machineAdaptorTwo = drawable;
//                }
//            }
//        }
//    }
//
//    MachineDlg dlg(GetParent(), machineAdaptorOne, machineAdaptorTwo);
//
...
/**
 * Change working machine for machine adapter one
 * @param event The menu event
 */
void ViewEdit::OnMachineOne(wxCommandEvent& event)
{
    std::shared_ptr<Drawable> machineAdaptorOne;
    ...
    for(auto actor :GetPicture()->GetActors())
    {
        if(actor->GetName() == "MachineOne")
        {
            for(auto drawable : actor->GetDrawables())
            {
                if(drawable->GetName() == L"MachineAdapterOne")
                {
                    drawable->OnMachine(mParent,this);
                }
            }
        }
    }
...
/**
 * Change working machine for machine adapter two
 * @param event The menu event
 */
void ViewEdit::OnMachineTwo(wxCommandEvent& event)
{
    std::shared_ptr<Drawable> machineAdaptorOne;
