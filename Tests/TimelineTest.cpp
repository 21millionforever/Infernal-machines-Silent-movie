...
TEST(TimelineTest, FrameRate)
{
    ...
    // Default number of frames
    ASSERT_EQ(20, timeline.GetFrameRate());
...
TEST(TimelineTest, GetDuration)
{
    ...
    // Default value
    ASSERT_NEAR(30, timeline.GetDuration(), 0.0001);

    // Changed duration
    timeline.SetFrameRate(375);
    ASSERT_NEAR(600.0 / 375.0, timeline.GetDuration(), 0.0001);
...
TEST(TimelineTest, GetCurrentFrame)
{
    ...
    ASSERT_EQ(int(9.27*20), timeline.GetCurrentFrame());
}
